...
TOPOLOGY_CHECKER_PATH = (
    SCRIPT.parents[2] / "start-work" / "scripts" / "check_git_topology.py"
)
...
def load_topology_checker():
    ...
    if spec is None or spec.loader is None:
        raise publish_error(
            ...
            responsibility="current-agent",
            disposition="fix-and-retry",
            required_action="Restore the start-work topology checker.",
    ...
    try:
        ...
        raise publish_error(
            ...
            responsibility="current-agent",
            disposition="fix-and-retry",
            required_action="Repair the start-work topology checker.",
            resume_when="The checker loads from the same candidate tree.",
        ) from error
    return module
...
def read_remote_destination_state(
    ...
    return any(
        line.partition("\t")[2] == policy.destination_ref
...
def push_revision(
    ...
    *,
    ...
    read_remote_destination_state(policy, repository, tools, private_key)
    command = [
        str(tools.git),
        "-c",
        f"core.sshCommand={ssh_command(policy, tools, private_key)}",
...
def parser() -> argparse.ArgumentParser:
    result = DiagnosticArgumentParser(
        description=__doc__, diagnostic_source="push-repository"
    )
