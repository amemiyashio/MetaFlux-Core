...
| [manage-host-privilege](manage-host-privilege/SKILL.md) | Active | Managing sudo or su, bounded host package escalation, persistent non-secret grants, or privileged driver debugging |
...
| Sudo, su, root helper, confirmed-gap host package, or privileged driver operation | `$manage-host-privilege` + `$manage-toolchain` for Nix-gap proof + the owning domain skill | Host privilege owns bounded elevation, authorization, and revocation; toolchain owns Nix-first proof; the domain skill owns operation semantics and evidence |
...
| M0110 vfio-user DMA unmap versus BAR/MSI-X completion | `$linux-device-driver-uapi` + `$gpu-virtualization-vfio-user` + `$pcie-vpci-device-model` | Linux owns pin/UAPI lifetime; vfio-user owns wire/DMA drain; PCI owns regions and notification; no replacement lifecycle is involved |
| M0120 vfio-user reset with generation replacement | `$linux-device-driver-uapi` + `$gpu-virtualization-vfio-user` + `$pcie-vpci-device-model` + `$device-lifecycle-resilience` | The three layer owners fence old resources; lifecycle alone owns candidate, retirement/epoch, and replacement commit |
...
| M0110 UAPI/device/BAR schema freeze | `$runtime-contracts-registry` + `$linux-device-driver-uapi` + `$gpu-virtualization-vfio-user` + `$pcie-vpci-device-model` | Runtime/contracts owns the canonical schema; each layer owns generated integration and behavior |
