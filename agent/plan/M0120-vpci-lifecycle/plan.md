...
# M0120: Lifecycle and vPCI Presentation

## Outcome

Add one authoritative reset/hotplug lifecycle to the static M0110 transports,
...
M0110 descriptor, base ioctl/mmap UAPI, BAR layout, and steady-state ring remain
unchanged.

[M0110](../M0110-kernel-guest-transport/plan.md) completion is a prerequisite.
...
M0120 adds only lifecycle transactions and milestone evidence around it.
...
- The lifecycle extension is authored once under
  ...
  manifest imports the frozen M0110 root manifest by content hash; the M0110 root
  never imports the extension or changes bytes. W0121 must run the repository
...
- M0100 memfd, M0110 local cdev, and M0110 guest vfio-user share one generation
...
- Authoritative nodes remain the M0110-owned `/dev/metafluxctl` and
  `/dev/metafluxN`. M0120 adds no functional UVM node or base ioctl/mmap surface.
...
- Bare-metal vroot is presentation-only, default-off, and has a separate package
  promotion gate. Its failure does not delay the lifecycle core or M0130.
...
| --- | --- |
| [W0121](work/W0121-lifecycle-model.md) | Idempotent lifecycle model, publication transactions, and ABI 0.x fixtures |
| [W0122](work/W0122-existing-transports.md) | Reset/hotplug on memfd, cdev, and guest vfio-user |
| [W0123](work/W0123-core-qualification.md) | 1,000-cycle core qualification and lifecycle v1 freeze |
| [W0124](work/W0124-experimental-vroot.md) | Default-off software root, presentation, pre-bind, and aliases |
| [W0125](work/W0125-performance-release.md) | Lifecycle-core performance, packaging, and release evidence independent of vroot |
...
- M0110 warm-dispatch latency remains within its original local and guest bounds.
...
- Config/sysfs queries and 1 Hz `nvidia-smi` remain inside the M0100 compute
...
Items 1-2 freeze only after W0123 core qualification. Item 3 freezes only for
the experimental vroot package after W0124 promotion evidence. Items 6-7 gate
W0124 package promotion; item 7 also gates W0125 core release packaging.

## Definition of Done

The lifecycle core is complete when M0110 remains green with no data-plane ABI
...
`mf_admin_lifecycle_v1` freezes only after qualification. M0130 release
