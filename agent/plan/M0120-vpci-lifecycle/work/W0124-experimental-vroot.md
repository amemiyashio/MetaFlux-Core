...
imports the frozen M0110 base and qualified lifecycle-extension manifests by
...
fails qualification. Promotion consumes the W0123-qualified lifecycle hash.
...
Canonical nodes remain the M0110-owned `/dev/metafluxctl` and `/dev/metafluxN`;
...
- [ ] Produce packaging-owned `metaflux-vroot-dkms` and
  ...
  depend on the vroot package, never the M0110 base `metaflux-vpci-dkms`
  artifact.
...
is at most 0.5%, 1 Hz `nvidia-smi` remains inside the M0100 compute-impact budget,
