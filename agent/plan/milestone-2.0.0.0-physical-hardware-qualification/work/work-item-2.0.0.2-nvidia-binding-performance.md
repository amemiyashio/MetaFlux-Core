...
performance budgets to binding for `v2.0.0`. The physical device is the native
...
- [ ] Compare AMD and Intel reference roles as separate artifacts and promote
  the inherited provisional budgets to binding only after every required row
  passes.
...
milestone-2.0.0.0 plan records `budgets: binding` with the exact evidence revisions.
