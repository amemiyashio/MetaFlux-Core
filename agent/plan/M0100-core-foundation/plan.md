...
M0100 establishes the registry, client protocol, and backend ABI required by that
model; it does not implement the M0110 kernel/guest transports.
...
- Uncontended active memfd queue: at most one wake syscall per dispatch, zero
  ...
  M0110 doorbell transports (cdev, vfio-user).
...
M0100 / `v0.1.0` is complete only when every in-scope workstream exit gate and
