...
| Path | Ownership |
| --- | --- |
| `contracts/plugin/backend/v1/` | milestone-0.1.0.0 backend C function table |
| `contracts/protocol/client/v1/` | milestone-0.1.0.0 provider/client negotiation |
...
start at ABI `0.x`; the inherited milestone-0.1.0.0 descriptor is already frozen. v1 freezes
...
until milestone-0.1.2.0 qualification.
...
- [x] Generate all language/layout assertions and byte fixtures. Added
  ...
  size, alignment, and field metadata). C/C++ golden bytes and static_asserts
  already generated by `--generate-header`. Rust and kernel KUnit fixtures
  remain open.
