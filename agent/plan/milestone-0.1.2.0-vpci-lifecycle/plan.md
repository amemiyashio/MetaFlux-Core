...
Add one authoritative reset/hotplug lifecycle to the static milestone-0.1.1.0 transports,
...
milestone-0.1.1.0 descriptor, base ioctl/mmap UAPI, BAR layout, and steady-state ring remain
unchanged.

[milestone-0.1.1.0](../milestone-0.1.1.0-kernel-guest-transport/plan.md) completion is a prerequisite.
...
milestone-0.1.2.0 adds only lifecycle transactions and milestone evidence around it.
...
- The lifecycle extension is authored once under
  ...
  manifest imports the frozen milestone-0.1.1.0 root manifest by content hash; the milestone-0.1.1.0 root
  never imports the extension or changes bytes. work-item-0.1.2.1 must run the repository
...
- milestone-0.1.0.0 memfd, milestone-0.1.1.0 local cdev, and milestone-0.1.1.0 guest vfio-user share one generation
...
- PCI class remains `0x120000`; CI VID/DID remains `0x4D46:0x0001` and release
  ...
  claim (decision-0008). Before scan, the kernel verifies the MetaFlux driver and keeps
...
- Authoritative nodes remain the milestone-0.1.1.0-owned `/dev/metafluxctl` and
  `/dev/metafluxN`. milestone-0.1.2.0 adds no functional UVM node or base ioctl/mmap surface.
...
- Bare-metal vroot is presentation-only, default-off, and has a separate package
  promotion gate. Its failure does not delay the lifecycle core or milestone-0.1.3.0.
...
| --- | --- |
| [work-item-0.1.2.1](work/work-item-0.1.2.1-lifecycle-model.md) | Idempotent lifecycle model, publication transactions, and ABI 0.x fixtures |
| [work-item-0.1.2.2](work/work-item-0.1.2.2-existing-transports.md) | Reset/hotplug on memfd, cdev, and guest vfio-user |
| [work-item-0.1.2.3](work/work-item-0.1.2.3-core-qualification.md) | 1,000-cycle core qualification and lifecycle v1 freeze |
| [work-item-0.1.2.4](work/work-item-0.1.2.4-experimental-vroot.md) | Default-off software root, presentation, pre-bind, and aliases |
| [work-item-0.1.2.5](work/work-item-0.1.2.5-performance-release.md) | Lifecycle-core performance, packaging, and release evidence independent of vroot |
...
```text
start static milestone-0.1.1.0 guest path
...
- milestone-0.1.1.0 warm-dispatch latency remains within its original local and guest bounds.
...
- Config/sysfs queries and 1 Hz `nvidia-smi` remain inside the milestone-0.1.0.0 compute
...
`mf_admin_lifecycle_v1` freezes only after qualification. milestone-0.1.3.0 release
