...
1. `kernel/vroot/README.md` spoke of an optional NVIDIA identity while
   ...
   kernel ABI claims. M0120 already contained half of the intent
   ...
   The registration/legal review requirement was added to M0120 decision 4
   because a synthetic NVIDIA VID at release is a compliance question, not an
   engineering one.
2. The M0100 provider closure gates mathematically required glibc >= 2.34
   (threads and dlopen symbols live in libpthread/libdl below 2.34) while M0100
...
3. The zero-syscall active-queue gate was physically unsatisfiable on the v0.1
   ...
   requires a syscall from someone. M0110 solves this with BAR2/ioeventfd on the
...
4. M0100 carried precise numeric budgets with no measurement evidence and no
...
5. "One authoritative logical-device view" read as global enumeration while
   W0106 coexistence is namespace-based isolation. The view is now scoped
   per managed domain.
...
- `agent/plan/M0100-core-foundation/plan.md` (decision 1 closure, DT_NEEDED
  universe, memfd wake budget, budgets status)
- `agent/plan/M0120-vpci-lifecycle/plan.md` (D0008 cross-reference, legal
...
- M0110 data-plane v1 freeze still precedes any device-class backend; the
  extension-namespace decision (M0110 decision 1) is the designated mitigation
  and was not altered.
...
- Worker spin/adaptive/sleep policy itself is deliberately not fixed by these
  amendments; it stays an implementation freedom until M0110 evidence exists.
