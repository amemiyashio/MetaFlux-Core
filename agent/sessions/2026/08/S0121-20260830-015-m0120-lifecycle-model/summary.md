...
view freeze without changing M0110 bytes.
...
- `tools/check-lifecycle-model.py`: deterministic validator and bounded DFS.

## Verification

| Command/gate | Result |
| --- | --- |
| Model checker | Passed: 949 states, 4,012 transitions, 326 complete sequences, 13 direct boundary checks |
...
- Deterministic lifecycle checker -> `tools/check-lifecycle-model.py` (949-state bounded run)
...
- W0121 remains Active until lifecycle adapters consume the model and the
  broader provider-view, fault, and qualification gates pass.

## Handoff

Run `python3 tools/check-lifecycle-model.py` with the W0121 root command after
the inputs are present; keep the M0110 base manifest unmodified.
