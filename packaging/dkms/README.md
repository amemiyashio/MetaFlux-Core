...
`metaflux-vpci-dkms` is staged by `packaging/dkms/stage-vpci-dkms.py` from the combined
...
`metaflux-vroot-dkms` is staged by `packaging/dkms/stage-vroot-dkms.py`, which freezes the
