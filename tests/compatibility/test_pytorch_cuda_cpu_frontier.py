...
class CpuFrontierEvidenceTests(unittest.TestCase):
    ...
    def test_repository_corpus_matches_pinned_profile(self) -> None:
        ...
        self.assertEqual(
            frontier.compiled_ptx(
                next(entry for entry in corpus["cases"] if entry["id"] == "matmul-rect-f32")
            ),
            frontier.ROOT
        ...
        self.assertEqual(
            frontier.compiled_ptx(
                next(entry for entry in corpus["cases"] if entry["id"] == "addmm-f32")
            ),
            frontier.ROOT
            / "plugins/compat/cuda/abi/driver/profiles/pytorch-cuda-cpu-v1/addmm-f32.ptx",
        )
        self.assertEqual(
            frontier.compiled_ptx(
                next(entry for entry in corpus["cases"] if entry["id"] == "linear-bias-f32")
            ),
            frontier.ROOT
