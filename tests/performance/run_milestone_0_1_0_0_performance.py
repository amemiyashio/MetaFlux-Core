...
def qualify(
    ...
    return {
        "scope": "milestone-0.1.0.0 core performance subset; full work-item-0.1.0.6 acceptance requires every listed check",
        ...
        "milestone-0.1.0.0-performance-gate-status": full_status,
        "checks": checks,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Archive reproducible milestone-0.1.0.0 CPU-backed performance evidence"
    )
...
def main() -> int:
    ...
    try:
        ...
        if managed_requested:
            ...
            with tempfile.TemporaryDirectory(prefix="metaflux-milestone-0.1.0.0-performance-") as temporary:
                root = Path(temporary)
    ...
    print(
        f"milestone-0.1.0.0 performance evidence: {results_path} "
        f"gate={qualification['milestone-0.1.0.0-performance-gate-status']}"
    )
    if fatal_error is not None:
        print(f"milestone-0.1.0.0 performance error: {fatal_error}")
        return 1
    if args.mode == "binding-reference" and qualification["milestone-0.1.0.0-performance-gate-status"] != "pass":
        return 2
    return 0
