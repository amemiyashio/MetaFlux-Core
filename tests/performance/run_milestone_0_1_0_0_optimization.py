...
def git_source_identity(repository: Path) -> dict[str, Any]:
    ...
    if status:
        ...
        raise QualificationError(
            "milestone-0.1.0.0 optimization qualification requires a clean Git worktree; "
...
def run_pgo_stage(
    ...
    merged_profile = evidence_root / "milestone-0.1.0.0-measured.profdata"
    recorder.run(
        "pgo-merge",
        [
            tools["llvm_profdata"],
...
def main() -> int:
    ...
    try:
        ...
        print(f"milestone-0.1.0.0 optimization evidence: {report_path} status={report['status']}")
        for error in errors:
