...
static int mf_load_symbol(void* library, const char* name, void* destination,
  ...
  if (error != (const char*)0 || symbol == (void*)0 || destination_size != sizeof(symbol)) {
    (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: dlsym(%s) failed: %s\n", name,
...
static int mf_report_cuda_failure(CUresult result, const char* expression, int line) {
  (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: %s failed at line %d with CUresult %d\n", expression,
                line, (int)result);
  return 1;
}
...
int main(int argc, char** argv) {
  ...
  if (mf_cpu_placement_matches(&arguments, &actual_cpu, &affinity_count) != 0) {
    (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: CPU affinity/NUMA placement mismatch\n");
    return 1;
  }
  ...
  if (cuda_library == (void*)0) {
    (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: dlopen(%s) failed: %s\n", arguments.cuda_library,
                  dlerror());
    goto cleanup;
  }
  ...
  if (!found_device) {
    (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: requested BDF %s is not visible\n",
  ...
  if (memcmp(source_host, destination_host, copy_bytes) != 0 ||
    ...
    (void)fprintf(stderr, "milestone-0.1.0.0-native-copy: copy correctness or placement mismatch\n");
    goto cleanup;
  }
