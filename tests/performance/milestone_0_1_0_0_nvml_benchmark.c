...
static int mf_report_nvml_failure(nvmlReturn_t result, const char* expression, int line) {
  (void)fprintf(stderr, "milestone-0.1.0.0-nvml-benchmark: %s failed at line %d: %s\n", expression, line,
...
int main(int argc, char** argv) {
  ...
  if (device_count == 0) {
    (void)fprintf(stderr, "milestone-0.1.0.0-nvml-benchmark: no managed NVML device was published\n");
    goto cleanup;
  }
