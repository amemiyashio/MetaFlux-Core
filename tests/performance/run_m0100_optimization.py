...
def git_source_identity(repository: Path) -> dict[str, Any]:
    ...
    if status:
        ...
        raise QualificationError(
            "M0100 optimization qualification requires a clean Git worktree; "
...
WORK_DIRECTORY_MARKER = ".metaflux-m0100-optimization-work"


def prepare_work_directory(requested: Path | None) -> Path:
    if requested is None:
        path = Path(tempfile.mkdtemp(prefix="metaflux-m0100-optimization-"))
    else:
        path = requested.resolve()
        ensure_fresh_output(path)
    (path / WORK_DIRECTORY_MARKER).write_text("owned by run_m0100_optimization.py\n", encoding="utf-8")
    return path
...
def run_pgo_stage(
    ...
    merged_profile = evidence_root / "m0100-measured.profdata"
    recorder.run(
        "pgo-merge",
        [
            tools["llvm_profdata"],
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and measure M0100 PGO, O2/O3, fuzz, and soak evidence"
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"M0100 optimization evidence: {report_path} status={report['status']}")
        for error in errors:
