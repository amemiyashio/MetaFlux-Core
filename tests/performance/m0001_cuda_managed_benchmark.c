...
static int mf_report_cuda_failure(CUresult result, const char* expression, int line) {
  ...
  (void)fprintf(stderr, "m0001-cuda-benchmark: %s failed at line %d: %s (%s)\n", expression, line,
                name, description);
  return 1;
}
...
int main(int argc, char** argv) {
  ...
  if (device_count < 1) {
    (void)fprintf(stderr, "m0001-cuda-benchmark: no managed CUDA device was published\n");
    goto cleanup;
  }
  ...
  for (index = 0; index < MF_BENCHMARK_ADD_ELEMENT_COUNT; ++index) {
    if (output[index] != left[index] + right[index]) {
      (void)fprintf(stderr, "m0001-cuda-benchmark: Add mismatch at element %" PRIu32 "\n", index);
      goto cleanup;
    }
  }
  if (memcmp(copy_source_host, copy_destination_host, copy_bytes) != 0) {
    (void)fprintf(stderr, "m0001-cuda-benchmark: managed copy mismatch\n");
    goto cleanup;
  }
