...
def qualify(
    ...
    return {
        "scope": "M0100 core performance subset; full W0106 acceptance requires every listed check",
        ...
        "m0100_performance_gate_status": full_status,
        "checks": checks,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Archive reproducible M0100 CPU-backed performance evidence"
    )
...
def main() -> int:
    ...
    try:
        ...
        if managed_requested:
            ...
            with tempfile.TemporaryDirectory(prefix="metaflux-m0100-performance-") as temporary:
                root = Path(temporary)
    ...
    print(
        f"M0100 performance evidence: {results_path} "
        f"gate={qualification['m0100_performance_gate_status']}"
    )
    if fatal_error is not None:
        print(f"M0100 performance error: {fatal_error}")
        return 1
    if args.mode == "binding-reference" and qualification["m0100_performance_gate_status"] != "pass":
        return 2
    return 0
