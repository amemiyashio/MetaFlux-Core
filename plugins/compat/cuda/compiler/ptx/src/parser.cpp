  ...
  void parse_header() {
    ...
    if (!failed_ && version.text != "9.0") {
      fail(DiagnosticCode::PtxUnsupportedVersion, version,
           "the M0001 manifest is pinned to PTX ISA 9.0", std::string(version.text));
      return;
    }
    take_word(".target");
    const auto target = take(TokenKind::Word, "PTX target");
    if (!failed_ && target.text != "sm_70") {
      fail(DiagnosticCode::PtxUnsupportedTarget, target, "the M0001 manifest requires target sm_70",
    ...
    if (!failed_ && address_size.text != "64") {
      fail(DiagnosticCode::PtxTypeMismatch, address_size,
           "the M0001 manifest requires 64-bit global addresses", std::string(address_size.text));
    }
  }
