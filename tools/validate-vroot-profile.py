...
def header_text(profile: dict[str, Any], image: bytes, writable: bytes) -> str:
...
static const uint8_t mf_vroot_profile_config_template[MF_VROOT_PROFILE_CONFIG_SIZE] = {
%s
};

static const uint8_t mf_vroot_profile_writable_mask[MF_VROOT_PROFILE_CONFIG_SIZE] = {
%s
};

#endif
""" % (rows(image), rows(writable))
...
def main() -> int:
    ...
    try:
        ...
        if arguments.generate_header is not None:
            ...
            output.write_text(header_text(profile, image, writable), encoding="utf-8")
