...
subsequent PCI rescan. While the bridge is online the value may grow; the
parameter setter updates the protected count and requests a root-bus rescan
after releasing the config lock, so newly enabled functions become discoverable
without touching the config callback. A decrease is rejected because the
current kernel projection has no matching device-removal transaction; this
keeps config callbacks and the PCI device list consistent. The root bus
`remove` path handles complete disappearance. Config access uses fixed storage,
a spinlock, and the generated writable mask; no allocation, sleep, RPC, or
userspace access occurs in the callback.
